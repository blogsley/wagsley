...
class Command(LabelCommand):
    ...
    async def import_entry(self, title, content, items, item_node):
        ...
        # Create page
        try:
            page = await sync_to_async(EntryPage.objects.get)(slug=slug)
            ...
            page = await sync_to_async(EntryPage)(
                title=title,
                body=content,
                block=block,
            ...
            await sync_to_async(self.blogpage.add_child)(instance=page)
            revision = await sync_to_async(self.blogpage.save_revision)()
            await sync_to_async(revision.publish)()
        await sync_to_async(self.import_entry_tags)(item_node.findall('category'), page)
        await sync_to_async(self.import_entry_categories)(item_node.findall('category'), page)
        # Import header image
        image_id = await sync_to_async(self.find_image_id)(item_node.findall(u'{{{0:s}}}postmeta'.format(self.WP_NS)))
        if image_id:
            await sync_to_async(self.import_header_image)(page, items, image_id)
        await sync_to_async(page.save)()
        await sync_to_async(page.save_revision)(changed=False)
    ...
    async def import_entries(self, items):
        ...
        for item_node in items:
            ...
            if post_type == 'post' and content and title:
                ...
                link = item_node.find('guid').text
                #page = await self.browser.newPage()
                print(link)
                await self.browser_page.goto(link)
                element = await self.browser_page.querySelector('div.entry-content')
                #content = await page.evaluate('(element) => element.innerHTML', element)
                content = await self.browser_page.evaluate('(element) => element ? element.innerHTML : element', element) or 'Empty'
                soup = bs4.BeautifulSoup(content, 'html5lib')
                content = soup.prettify()

                self.stdout.write(u'\t{0:s}'.format(title))
                content = await sync_to_async(self.process_content_image)(content)
                await self.import_entry(title, content, items, item_node)
